<template>
	<view class="admin-floor">
		<view class="admin-floor-top">
			<u-search :showAction="true" placeholder="请输入广告机名称" @search="handleFloorList()" @custom="handleFloorList()"
		...
		<view class="admin-floor-center">
			<view v-show="indexList.length" v-for="(item, index) in indexList" :key="index" class="examine-list-single">
				<view class="examine-list-single-text">广告机名称：{{item.deviceName}}</view>
		...
		<u-popup :customStyle="propStyle" :round="10" @close="show = false" mode="center" :show="show">
			<view class="floor-popup">
				<view>是否要删除：{{selectFloor.deviceName}}</view>
			</view>
	...
	export default {
		...
		methods: {
			handleFloorList() {
				...
				deviceAdnodeList(pram).then(res => {
					if (res.code === 0) {
			...
			editFloor(item) {
				if (!this.hasLogin) {
					...
					uni.$u.route('/pages/admin/admin_device_adnode/admin_device_adnode_edit?id=' + item.id)
				}
			},
			delFloor(item) {
			...
			delFloorSure() {
				...
				deleteDeviceAdnode(pram).then(res => {
					if(res && res.code === 0){
						this.show = false
						uni.showToast({
							title: '删除成功'
