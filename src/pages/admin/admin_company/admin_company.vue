<template>
	<view class="admin-floor">
		<view class="admin-floor-top">
			<u-search :showAction="true" placeholder="请输入公司名称" @search="handleFloorList()" @custom="handleFloorList()"
		...
		<view class="admin-floor-center">
			<view v-show="indexList.length" v-for="(item, index) in indexList" :key="index" class="examine-list-single">
				<view class="examine-list-single-text">{{item.name}}</view>
				<u-button :custom-style="popupButtomButton" @click="codeFloor(item)" size="small" type="success"
					:plain="true" text="二维码">
				</u-button>
		...
		methods: {
			...
			codeFloor(item){
				...
				setTimeout(() => {
					...
					var qr = new UQRCode();
					// 设置二维码内容
					qr.data = mix.domainUrl + 'pages/login/register?data=[2,' + item.id +
						',0]'
			...
			editFloor(item) {
				if (!this.hasLogin) {
					...
					uni.$u.route('/pages/admin/admin_company/admin_company_edit?id=' + item.id)
				}
			},
			delFloor(item) {
