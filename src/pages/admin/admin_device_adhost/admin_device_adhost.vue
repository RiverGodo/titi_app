<template>
	<view class="admin-floor">
		<view class="admin-floor-top">
			<u-search :showAction="true" placeholder="请输入主机名称" @search="handleFloorList()" @custom="handleFloorList()"
		...
		<view class="admin-floor-center">
			<view v-show="indexList.length" v-for="(item, index) in indexList" :key="index" class="examine-list-single">
				<view class="examine-list-single-text">主机名称：{{item.deviceName}}</view>
	...
	export default {
		...
		methods: {
			handleFloorList() {
				...
				deviceAdhostList(pram).then(res => {
					if (res.code === 0) {
			...
			editFloor(item) {
				if (!this.hasLogin) {
					...
					uni.$u.route('/pages/admin/admin_device_adhost/admin_device_adhost_edit?id=' + item.id)
				}
			},
			delFloor(item) {
			...
			delFloorSure() {
				...
				deleteDeviceAdhost(pram).then(res => {
					if(res && res.code === 0){
						this.show = false
						uni.showToast({
							title: '删除成功'
