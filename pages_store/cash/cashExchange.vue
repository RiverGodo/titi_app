		...
		<view class="round_head_m">
			<view class="ex">
				<view class="ex_t w_flex_center">
					<cash-back :price="item.price" class="ex_t_l"></cash-back>
					<view class="ex_t_r">
				...
				<view class="ex_m w_flex_center">
					<view class="ex_m_l w_flex_center">
						...
						<text v-if="points >= cashDetial.costBean * cashNumber"
							style="color: #969696;">（{{points}}现金豆）</text>
						<text v-else style="color: #969696;">（当前现金豆不足）</text>
					</view>
				</view>
				<view class="ex_e">
					小计（现金豆）：
					<text style="color: #FC5531;font-size: 32rpx;">{{cashDetial.costBean * cashNumber}}</text>
				</view>
			</view>
			<view class="ex_btn w_flex_center">
				<view class="ex_btn_l">
					合计：
					<text style="color: #FC5531;font-size: 36rpx;">{{cashDetial.costBean * cashNumber}}现金豆</text>
				</view>
				<view class="ex_btn_m">共{{cashNumber}}件</view>
				<view v-if="points >= cashDetial.costBean" style="background: #FC5531;" @click="handleCreateOrder()"
		...
		<uni-popup ref="popup" type="bottom">
			<view class="popup_m">
				...
				<view v-show="popupStatus === 2">
					<view class="popup_m_title">如何获得现金豆</view>
					<view class="popup_m_m">
						...
						<view class="popup_m_m_b w_flex_center">
							<view class="popup_m_m_b_s">1.看广告送现金豆</view>
							<view class="popup_m_m_b_s">2.买广告卡送现金豆</view>
							<view class="popup_m_m_b_s">3.分享送现金豆</view>
						</view>
					</view>
					<view class="popup_m_l">
						<view class="popup_m_l_t">现金豆如何使用?</view>
						<view class="popup_m_l_s">
							1.梯梯助手现金豆与现金按照100:1的比例兑换</br>
							2.现金券可用现金豆全额兑换，普通商品现金豆抵扣金额不可超过该订单总金额的10%</br>
							3.每人每天只可使用1次现金豆用于购买时抵扣现金</br>
							4.每个用户仅限使用1个账号参与现金豆活动(同设备、微信号、支付账号均为1个账号)</br>
	...
	export default {
		...
		methods: {
			...
			async init(id) {
				...
				let res3 = await this.$get('/seller/wallet/balance');
				this.cashDetial = res.data
				this.shopData = res2.data
				this.points = res3.data.bean
			},
			async handleCreateOrder() {
				...
				this.navTo("/pages_store/cash/cashSuccess",{login:true})
			},
			eventChange(data) {
