	...
	<view class="abl">
		<view class="abl_t w_flex_center">
			<view class="abl_t_s ablts_l">
				<view class="abl_t_s_t">{{wallet.money}}</view>
				<view class="abl_t_s_b">可用余额</view>
			</view>
		</view>
		<view class="abl_m">
			<view class="abl_m_t">余额明细</view>
			<view class="abl_m_m">
				<mescroll-uni height="69%" :bottombar="false" ref="mescrollRef" @init="mescrollInit"
					...
					<view v-for="(item,index) in dataList" :key="index" class="abl_m_m_s">
						...
						<view class="abl_m_m_s_b w_flex_center">
							...
							<text>余额:{{item.latestUsable | price }}</text>
						</view>
					</view>
				</mescroll-uni>
			</view>
		...
		<view class="abl_b">
			<view class="abl_b1" @click="navTo('/pages_store/money/withdraw')">
				<view class="abl_b1_m">
					提现
				</view>
			</view>
	...
	export default {
		...
		methods: {
			...
			async loadDataList(e) {
				const res = await this.$get('/seller/seller-money-log/listBalanceLog', {
					currentPage: e.num,
					pageSize: e.size,
				}, {}, () => {
