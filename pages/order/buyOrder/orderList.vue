	...
	<view class="ol">
		...
		<view class="ol_m">
			<mescroll-uni height="94%" :bottombar="false" ref="mescrollRef" @init="mescrollInit" @down="downCallback"
				...
				<view v-for="(item, index) in cashList" @click="navTo('/pages/order/buyOrder/orderDetial?id=' + item.id, {login: true})"
					...
					<view class="operation_box">
						<view @click.stop="navTo('/pages/ad/adExchange?id=' + item.id)" v-if="item.status == 0" class="ol_m_r">
							去支付
						</view>
					</view>
				</view>
