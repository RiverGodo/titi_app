<template>
    ...
    <text class="log" @click="navTo('/pages/money/withdrawLog')">提现记录</text>
  </view>
</template>
...
<script>
    ...
    data() {
      ...
      this.getParams();
		...
		methods: {
			async getParams() {
				const res = await this.$get("/withdraw-log/withdrawParams");
        ...
        if(this.num * 1 < this.params.min *1) {
          ...
          return;
				...
				const res = await this.$post("/withdraw-log/withdraw", {
					amount: this.num,
					code: this.code,
				});
        if(res.code === -2){
        ...
        this.$util.msg("提现申请已提交");
				setTimeout(() => {
          this.navTo('/pages/money/withdrawLog');
        }, 1000);
			}
		}
	}
