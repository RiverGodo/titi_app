		...
		<view class="cash_detial_b">
			<view class="cash_detial_m">
				<view class="s1 w_flex_end">
					<cash-back :price="cashDetial.price" class="s1_img"></cash-back>
					<view class="s1_right">
						<view class="s1_r_t">
							{{cashDetial.name}}
						</view>
						<view class="s1_r_b w_flex_center">
							<view class="s1_r_b_l">{{cashDetial.costPoints}}积分</view>
				...
				<!-- 	<view class="s2_t1">
					...
					<view class="s2_t1">
						<view class="s2_title w_flex_center">
							...
							<view class="s2_title_r">每人{{cashDetial.perUserLimit}}张</view>
						</view>
					</view>
				</view>
				<view class="s3">
...
<script>
	export default {
		...
		methods: {
			handleConfirm(){
				this.navTo('/pages/cash/cashExchange?id=' + this.cashID,{login:true})
			},
			async init(id){
				let res = await this.$get('/seller/coupon/get', {id: id});
				let res2 = await this.$get('/seller/find', {id: res.data.sellerId});
