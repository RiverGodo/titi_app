	...
	<view class="cash_shop_detial">
		...
		<view :style="'background-image: url('+ backImg +');'" class="s_top">
			<view class="s_top_1 w_flex_center">
				...
				<view class="s_top_1_r">
					...
					<view @click="navTo('/pages/cash/shopDetial?id=' + shopData.id)" class="s_top_1_r_b">查看详情</view>
				</view>
			</view>
		...
		<view class="s_list">
			<mescroll-uni height="65%" ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="loadDataList"
				:up="upOption">
				<view v-for="(item,index) in cashList" :key="index" class="s_list_s w_flex_center">
					<cash-back @click="navTo('/pages/cash/cashDetial?id=' + item.id)" :price="item.price"
						class="s_list_s_i"></cash-back>
					<view @click="navTo('/pages/cash/cashDetial?id=' + item.id)" class="s_list_s_m">
					...
					<view @click="navTo('/pages/cash/cashExchange?id=' + item.id, {login: true})" class="s_list_s_r">
						兑换
					</view>
				</view>
			</mescroll-uni>
